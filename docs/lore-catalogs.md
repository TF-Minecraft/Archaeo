...
`docs/concepto.md` for this task.
...
| `artifacts.*.item` | Bukkit `Material` name (`GOLD_NUGGET`, `BONE`, …) | Falls back to `BRICK`. Not ItemsAdder / MMOItems ids |
...
11. `size-min ≤ size-max`. `item` is a Bukkit material. Stratum keys are still
